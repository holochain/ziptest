...
fn send_message(input: SendMessageInput)-> ExternResult<()> {
    debug!("Sending {:?} to {:?}", input.content, input.agents);
    let result = send_remote_signal(Message{stream_id:input.stream_id, content:input.content}, input.agents);
    debug!("send_remote_signal called with result {:?}", result);
    result
}

#[hdk_extern]
fn recv_remote_signal(message: Message) -> ExternResult<()> {
    debug!("recv_remote_signal called");
    let info = call_info()?;
    let signal = Signal::Message { stream_id: message.stream_id.clone(), content: message.content.clone(), from: info.provenance };
    debug!("with message:{:?} \ncalling emit_signal", message);
    let result = emit_signal(signal);
    debug!("emit_signal called with result {:?}", result);
    result
}
